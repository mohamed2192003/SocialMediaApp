...
class AuthService {
    ...
    async signup(data: SignupDTO): Promise<IUser> { // data has the type of SignupDTO which is inferred from the signupSchema that we defined in the auth.validation.ts file, this way we can ensure that the data that is being passed to the signup method has the correct structure and types, we are also defining the return type of the method as IUser which is the interface that defines the structure of the user documents in the database, this way we can ensure that the data that is being returned from the signup method has the correct structure and types.
    ...
    async login(data: LoginDTO, issuer?: string): Promise<{ result: HydratedDocument<IUser>, accessToken: string, refreshToken: string } | null> {  // data has the type of LoginDTO which has the properties email and password with their respective types and validation rules
